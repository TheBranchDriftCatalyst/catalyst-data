...
class ProcessingTracker:
    ...
    def _load(self) -> None:
        try:
            ...
            logger.info("Loaded %d processed hashes from %s", len(self._hashes), self._key)
        except Exception:
            self._hashes = set()
            logger.info("No existing tracker at %s — starting fresh", self._key)

    def filter_unprocessed(self, chunks: list) -> list:
        """Return only chunks whose content_hash is not yet processed."""
        return [c for c in chunks if getattr(c, "content_hash", "") not in self._hashes]
    ...
    def save(self) -> None:
        ...
        self._dirty = False
        logger.info("Saved %d processed hashes to %s", len(self._hashes), self._key)
