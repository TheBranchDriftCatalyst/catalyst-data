...
class Provenance(BaseModel):
    ...
    temporal_start_ms: int | None = Field(default=None, description="Milliseconds into source media where extraction starts (audio/video only)")
    temporal_end_ms: int | None = Field(default=None, description="Milliseconds into source media where extraction ends (audio/video only)")
    speaker_label: str | None = Field(default=None, description="Session-local speaker ID from diarization (e.g. SPEAKER_00)")
